...
int main(void) {
    ...
    MsgPlayerState msg;
    Phase1RRealHookEvidenceConfig cfg;
    Phase1RRealHookEvidenceState state;
    ...
    callback_cfg.install_config.expected_local_player_id = 401;
    ...
    assert(msg.player_id == 401);

    memset(&proxy_cfg, 0, sizeof(proxy_cfg));
    proxy_cfg.local_player_id = 1;
    proxy_cfg.default_proxy_base_form_id = 0xA157u;
    ...
    cfg.validation_config.local_player_id = 401;
    cfg.validation_config.remote_player_id = 401;
    cfg.validation_config.require_toolchain_match = true;
    cfg.validation_config.require_callback_validation = true;
    cfg.validation_config.require_proxy_validation = true;
    cfg.validation_config.require_remote_runtime_present = true;
    cfg.validation_config.require_scene_present = true;
    cfg.validation_config.require_driver_present = true;
    cfg.validation_config.callback_validation_config.expected_local_player_id = 401;
    cfg.validation_config.callback_validation_config.min_callback_forward_count = 1;
    cfg.validation_config.callback_validation_config.min_callback_accept_count = 1;
    cfg.validation_config.callback_validation_config.require_runtime_profile_supported = true;
    cfg.validation_config.callback_validation_config.require_callback_registered = true;
    cfg.validation_config.callback_validation_config.require_provider_ready = true;
    cfg.validation_config.callback_validation_config.require_captured_state = true;
    cfg.validation_config.callback_validation_config.require_expected_player_id_match = true;

    assert(phase1r_rhe_collect(&cfg));
    state = phase1r_rhe_state();
    assert(state.collected);
    assert(state.evidence_ready);
    assert(state.failure == PHASE1R_RHE_FAIL_NONE);
    assert(state.toolchain_ok);
    assert(state.callback_ok);
    assert(state.proxy_ok);
    assert(state.harness_ok);
    assert(state.local_player_id == 401);
    assert(state.remote_player_id == 401);
    assert(strcmp(state.runtime_profile_name, "fo4_1_11_191_player_character_update_before_input") == 0);
    return 0;
}
