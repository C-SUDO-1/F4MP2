...
int main(void) {
    ...
    assert(f4mp_spawn_service(&svc, "7785"));
    usleep(200000);

    proxy_runtime_init();
    assert(ptc_open(&a, "127.0.0.1", 7785));
    assert(ptc_open(&b, "127.0.0.1", 7785));
    ...
    for (i = 0; i < 80; ++i) {
        ...
        if (remote && remote->position.x == 11.0f) break;
        usleep(10000);
    }
    remote = rpm_get_remote_player(ptc_local_player_id(&a));
    assert(remote != NULL);

    assert(ptc_send_disconnect(&a, DISC_NONE));
    for (i = 0; i < 80; ++i) {
        assert(ptc_poll_once(&b, 10, &applied));
        remote = rpm_get_remote_player(ptc_local_player_id(&a));
    ...
    assert(rpm_get_remote_player(ptc_local_player_id(&a)) == NULL);
