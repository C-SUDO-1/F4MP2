...
int main(void) {
    ...
    assert(s != 0);
    assert(s->target_position.x == 10.0f);
    assert(s->presented_position.x == 0.0f);

    fo4_scene_proxy_backend_stub_step();
    s = fo4_scene_proxy_backend_stub_get(2);
    assert(s->interpolation_tick_count == 1);
    assert(fabsf(s->presented_position.x - 5.0f) < 0.001f);
    assert(fabsf(s->presented_rotation.yaw - 45.0f) < 0.001f);

    fo4_scene_proxy_backend_stub_step();
    s = fo4_scene_proxy_backend_stub_get(2);
    assert(fabsf(s->presented_position.x - 7.5f) < 0.001f);
    assert(fabsf(s->presented_rotation.yaw - 67.5f) < 0.001f);
    return 0;
}
