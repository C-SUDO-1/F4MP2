...
int main(void) {
    ...
    cfg.observed_f4se_build_text = "0.7.7";
    ...
    assert(st.plugins_directory_present);
    assert(!st.f4se_log_present);
    ...
    assert(strcmp(st.site_prototype_name, "player_character_update_before_input") == 0);
    assert(strcmp(st.manifest_runtime_text, "1.11.191") == 0);
    assert(strcmp(st.manifest_f4se_build_text, "0.7.7") == 0);
    return 0;
}
